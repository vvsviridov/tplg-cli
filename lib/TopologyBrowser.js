...
class TopologyBrowser {
  ...
  async fdn(fdn) {
    this.poIds.length = 0
    const response = await requestWrapper(async () => this.conn.get(`${this.baseUrl}fdn/${fdn}`, {
      withCredentials: true
    }), 'Browsing to FDN...')
    const { fdn, poId } = response.data
    this.currentPoId = poId
    this.nextPoId = poId
    this.nextVariants = async (input) => await this.nextObjects(input)
    const response = await requestWrapper(async () => this.conn.get(`${this.baseUrl}network/${this.currentPoId}/subTrees`, {
      withCredentials: true
    }), 'Check FDN path...')
    if (response.data) {
      response.data.treeNodes.forEach((node) => {
