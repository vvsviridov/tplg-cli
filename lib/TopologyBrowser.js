...
class TopologyBrowser {
  ...
  async fdn(targetFdn) {
    this.poIds.length = 0
    const response1 = await requestWrapper(async () => this.conn.get(`${this.baseUrl}fdn/${targetFdn}`, {
      withCredentials: true
    }), 'Browsing to FDN...')
    ...
    this.nextPoId = poId
    this.nextVariants = async (input) => await this.nextObjects(input)
    const response2 = await requestWrapper(async () => this.conn.get(`${this.baseUrl}network/${this.currentPoId}/subTrees`, {
      withCredentials: true
    }), 'Building FDN path...')
    if (response2.data) {
      ...
      this.childrens = response2.data.treeNodes[0].childrens
    }
    return fdn
  }
