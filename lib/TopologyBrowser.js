...
class TopologyBrowser {
  ...
  async logout(){
    ...
    const response = await requestWrapper(this.conn, axiosConfig, 'Logout...')
    if (response.status === 200) {
  ...
  async initialPrompt() {
    ...
    const response = await requestWrapper(this.conn, axiosConfig, 'Starting Topology Browser...')
  ...
  async nextObjects(input){
    ...
    if (this.currentPoId !== this.nextPoId || !this.childrens) {
      ...
      const response = await requestWrapper(this.conn, axiosConfig)
      if (response.data.treeNodes) {
  ...
  async config(fdn) {
    ...
    const responseA = await requestWrapper(this.conn, axiosConfig, 'Reading Attributes...')
    ...
    const responseD = await requestWrapper(this.conn, axiosConfig, 'Reading Attributes Data...')
  ...
  async commit(fdn) {
    ...
    const response = await requestWrapper(this.conn, axiosConfig, 'Commiting Config...')
    if (response.data) {
  ...
  async fdn(fromFdn, targetFdn) {
    ...
    const response1 = await requestWrapper(this.conn, axiosConfig, 'Browsing to FDN...')
    ...
    const response2 = await requestWrapper(this.conn, axiosConfig, 'Building FDN path...')
    if (response2.data) {
  ...
  async alarms(fdn) {
    ...
    const response1 = await requestWrapper(this.conn, axiosConfig, 'Getting Alarms...')
    ...
    const response2 = await requestWrapper(this.conn, axiosConfig, 'Getting Alarms Data...')
